    ...
    if st.sidebar.checkbox("Login"):
        if username == 'admin' and password == 'admin1234':
            ...
            if navigation == "Results":
                st.subheader("Clients Profile")
                user_result = view_all_users()
                clean_database = pd.DataFrame(user_result, columns = ["username", "password"])
            ...
            create_usertable()
            result1 = login_user(username,password)
            if result1:
